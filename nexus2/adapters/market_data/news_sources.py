...
def get_finviz_headlines(symbol: str, limit: int = 10, days: int = 5) -> List[str]:
    """
    ...
    Args:
        ...
        days: Days to look back (filters by Date column)
    ...
    try:
        ...
        # Filter by date — previously this was missing, causing months-old
        # headlines (e.g. IPO announcements) to trigger false-positive rejections
        if "Date" in news_df.columns:
            try:
                cutoff = now_et() - timedelta(days=days)
                # Convert Date column to datetime for comparison
                news_df["Date"] = pd.to_datetime(news_df["Date"], errors="coerce")
                # Make cutoff offset-naive if dates are offset-naive
                if news_df["Date"].dt.tz is None:
                    cutoff = cutoff.replace(tzinfo=None)
                news_df = news_df[news_df["Date"] >= cutoff]
            except Exception as e:
                logger.debug(f"[Finviz] {symbol}: Date filtering failed ({e}), using unfiltered")
        
        headlines = news_df["Title"].head(limit).tolist()
        
        logger.debug(f"[Finviz] {symbol}: Found {len(headlines)} headlines (filtered to {days}d)")
        return headlines
...
def get_all_headlines(symbol: str, days: int = 5) -> List[str]:
    ...
    # Finviz (pass days for date filtering)
    for h in get_finviz_headlines(symbol, days=days):
        normalized = h.strip().lower()
        if normalized and normalized not in headlines_set:
            headlines_set.add(normalized)
            headlines_list.append(h.strip())
