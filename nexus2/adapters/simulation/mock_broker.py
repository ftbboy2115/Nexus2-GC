...
class MockBroker:
    ...
    def _fill_entry_order(
        ...
        if order_value > self._cash:
            print(f"[BROKER-TRACE] REJECT {symbol} x{quantity} @ ${fill_price:.2f} (need ${order_value:.2f}, have ${self._cash:.2f})")
            return BrokerOrder(
                client_order_id=client_order_id,
                broker_order_id=entry_order_id,
                symbol=symbol,
        ...
        print(f"[BROKER-TRACE] FILL {symbol} x{quantity} @ ${fill_price:.2f} trigger={entry_trigger} cash=${self._cash:.2f}")
